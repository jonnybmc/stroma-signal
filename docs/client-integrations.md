...
- The canonical navigation field is `meta.navigation_type` (typed as `SignalNavigationType` in the contracts package).
- `restore` and `prerender` stay in raw events, but default load-performance reporting excludes them.
