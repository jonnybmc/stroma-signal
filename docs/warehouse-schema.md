...
- Default Signal URL-builder templates exclude `navigation_type = restore` and `navigation_type = prerender` before computing coverage and percentiles. The **normalized** URL-builder additionally pre-filters `context_visibility_hidden_at_load = true` (background-tab loads) and preserves the excluded count via the `rs` (raw pre-filter sample size) and `xb` (excluded background sessions) report-URL params — invariant: `raw_sample_size === sample_size + excluded_background_sessions`. The **GA4** URL-builder cannot do this background-tab pre-filter: `context_visibility_hidden_at_load` is warehouse-only (the GA4 24-field event-param map is already at GA4's 25-param cap), so the GA4 path neither excludes hidden-at-load loads nor emits `rs`/`xb`. Customers that need this transparency must use the normalized recipe.
- The URL-builder templates only emit a URL when `sample_size >= 100` (the documented `SIGNAL_PREVIEW_MINIMUM_SAMPLE`). Emitted URLs carry a `b=<band>` parameter computed inline — `provisional` (100–499) or `stable` (≥ 500) — and the hosted `/r` cover renders a confidence-band note above the masthead when `b !== stable`. **Below 100 events** the URL-builder emits a diagnostic message in the `signal_report_url` column instead of a URL (`NO_EVENTS_IN_WINDOW: ...` for sample_size = 0; `SAMPLE_BELOW_RECOMMENDED_MINIMUM: captured N events ...` for 1–99) so operators get an actionable next step rather than a misleading half-baked artifact. The `b=preliminary` value still exists in the codec for in-browser preview URLs and `/build`-route URLs; pre-threshold-gate URLs (older snapshots without `b=` at all) decode cleanly via codec back-fill from `s=` (sample_size).
