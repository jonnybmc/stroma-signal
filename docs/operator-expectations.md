...
| **The `/api/v1/intent` demand-signal endpoint** | When a reader of `/r` clicks a CTA (e.g., "Keep me posted"), Stroma receives the intent payload server-side (event kind + capture id + optional email). User-agent is captured server-side for product-research purposes. Documented in the report's privacy posture. |
